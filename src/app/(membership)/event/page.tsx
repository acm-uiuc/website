...
const Event = () => {
  ...
  if (Object.keys(paidEventList).length === 0) {
    ...
    const isRunningOut =
      paidEventList['tickets_sold'] / paidEventList['event_capacity'] > 0.75;
    return (
      <Layout name={paidEventList['event_name']}>
        <div className="h-screen w-screen absolute top-0 left-0 flex flex-col items-center py-24">
          <Card className="max-w-[512px] mx-4 my-auto shrink-0">
            <CardHeader>
              <p className="font-bold">{paidEventList['event_name']} Signup</p>
            </CardHeader>
            <Divider />
            <CardBody className="gap-4">
              {paidEventList['eventImage'] ? (
                <img
                  alt={paidEventList['event_name'] + ' image.'}
                  src={paidEventList['eventImage']}
                />
              ) : null}
              <p>{paidEventList['eventDetails']}</p>
              <p>
                <b>Date:</b>{' '}
              ...
              <p>
                <b>Cost:</b> ${paidEventList['eventCost']['paid']} for paid
                ACM@UIUC members, ${paidEventList['eventCost']['others']} for
              ...
              <p>
                {isRunningOut ? (
                  <i>Tickets are running out, order soon!</i>
                ) : null}
              </p>
              <Input
                value={netId}
                ...
                color={
                  inputNetIdStatus === InputStatus.INVALID
                    ? 'danger'
                    : 'default'
                }
                errorMessage={
