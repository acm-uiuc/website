          ...
          <div className="container">
            ...
            <div className="grid md:grid-cols-2 gap-2 items-stretch pt-5 auto-rows-max">
              {itemsList.map((val: Record<string, any>) => (
                <Card
                  key={val['item_name']}
                  className="max-w-[512px] my-2 h-full container mt-auto flex"
                >
                  <CardHeader>
                    <p className="font-bold">{val['item_name']}</p>
                  </CardHeader>
                  <CardBody className="justify-center">
                    {val['item_image'] ? (
                      <img
                        alt={val['item_name'] + ' image.'}
                        src={val['item_image']}
                      />
                    ) : null}
                  </CardBody>
                  ...
                  {val['limit_per_person'] && val['limit_per_person'] > 0 ? (
                    <i>Limit {val['limit_per_person']} per person.</i>
                  ) : null}
                  <br className="mb-4" />
                  <Button
                    color="primary"
