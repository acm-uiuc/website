...
export default function CalendarControls({
  ...
  const [nextDisabled, setNextDisabled] = useState(
    moment(currDisplayDate).isSameOrAfter(maxRenderDistance),
  );
      ...
      <div className="sm:flex sm:flex-row md:grid lg:flex lg:grid-cols-2 w-full justify-between gap-x-4">
        <div className="flex items-center pb-1 lg:pb-0 lg:col-span-1">
          <div className="text-xl font-bold">
            {currView == Views.DAY
              ? getCurrentDate(currDisplayDate)
              : extractMonthAndYear(currDisplayDate)}
          </div>
        </div>
          ...
          <div>
            <Dropdown>
              ...
              >
                ...
                <DropdownItem key={Views.WEEK} className="hidden md:block">
                  Week View
                </DropdownItem>
                <DropdownItem key={Views.MONTH}>
                  Month View (default)
                </DropdownItem>
