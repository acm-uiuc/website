...
function createRecurringEvents(
  event: IEvent,
  repeatEntry: RepeatMappingEntry,
  repeatEnd: any,
) {
  const events = [];
...
const Events: React.FC<EventsProps> = ({
  ...
  useEffect(() => {
    ...
    if (id && filteredEvents) {
      const event = filteredEvents.find((event) => {
        if (date) {
          return (
            event.id === id && moment(event.start).format('YYYY-MM-DD') === date
          );
        }
        return event.id === id;
      });
  ...
  const dummyNav = (newDate: Date, view: View, action: NavigateAction) => {
    return;
  };
  ...
  useEffect(() => {
    ...
    const filteredEvents = events.filter(
      (event) =>
        ...
        (hostFilter
          ? event.host?.toLowerCase() === hostFilter.toLowerCase()
          : true),
    );
    // Convert the events to the format required by react-big-calendar
    const formattedEvents: (CalendarEvent | null)[] = filteredEvents.flatMap(
      (event) => {
        ...
        if (event.repeats && validRepeats.includes(event.repeats)) {
          const repeatEnd = event.repeatEnds
            ? moment(event.repeatEnds)
            : maxRenderDistance;
          return createRecurringEvents(
            event,
            repeatMapping[event.repeats],
            repeatEnd,
          );
        }
    ...
    const filteredFormattedEvents: CalendarEvent[] = formattedEvents.filter(
      (event) => event !== null,
    );
  ...
  return (
    <Skeleton
      isLoaded={calendarHeight != 0 || !events}
      ...
      {calendarHeight ? (
        ...
        <Calendar
          ...
          eventPropGetter={(event, start, end, isSelected) => {
            ...
            return {
              style: {
                backgroundColor: isSelected ? darkerColor : color,
