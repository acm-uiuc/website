...
export const coreApiBaseUrl =
  process.env.NEXT_PUBLIC_CORE_API_BASE_URL ?? 'https://core.acm.illinois.edu';
...
export async function fetchUpcomingEvents() {
  try {
    const rawDates = await eventsApiClient.apiV1EventsGet({
      upcomingOnly: true,
    });
...
export const checkIfSyncNeeded = async (
  ...
  const response = await genericApiClient.apiV1SyncIdentityIsRequiredGet({
